...
Drink.all.filter{|drink| drink.recipes.map{|recipe| posted_object[:mix].map{|ingred| recipe.ingredient_id == ingred[:ingredient_id] && recipe.parts == ingred[:parts]}.include? true}.all?{|item| item==true}}